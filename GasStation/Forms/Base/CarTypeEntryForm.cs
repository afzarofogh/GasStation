...
{
	public partial class CarTypeEntryForm : General.SuperForm
	{
		#region Variables
		/// <summary>
		/// Model
		/// </summary>
		Common.BLL.Entity.GasStation.Base__CarType	model;
		#endregion
		...
		/// Constructor
		/// </summary>
		public CarTypeEntryForm (Common.BLL.Entity.GasStation.Base__CarType model = null)
		{
			InitializeComponent ();
		...
		{
			if (null == model)
				model	= new Common.BLL.Entity.GasStation.Base__CarType ();
			else
			{
				...
				CommandResult	opResult	= lCarType.read (model);

				///TODO: CHECK ERRORS
			}

			// Fill Controls
			BaseBLL.General.FormModelHelper<Common.BLL.Entity.GasStation.Base__CarType>.fillControl (dataGroupBox, model);
		}
		...
		void saveButton_Click (object sender, EventArgs e)
		{
			...
			{ 
				// Fill Model
				BaseBLL.General.FormModelHelper<Common.BLL.Entity.GasStation.Base__CarType>.fillModel (dataGroupBox, model);
		...
		private void saveRecord ()
		{
			...
			if (model.id == 0)
			{
				...
				opResult = lCarType.create (model); 
				#endregion
			}
			else
			{
				...
				opResult = lCarType.update (model); 
				#endregion
			}
