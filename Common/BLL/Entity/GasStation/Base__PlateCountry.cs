...
{
	[Serializable]
	public class Base__PlateCountry : BaseBLL.Entity.BaseByViewId
	{
...
//
	...
	#region Relation - Plate (Has-Many relation)
		private System.Data.DataTable _get_Plate_plateCountryId;
		public System.Data.DataTable getPlate_plateCountryId
		{
			get
			{
				if ((_get_Plate_plateCountryId == null) && (AutoLoadForeignKeys))
					loadPlate_plateCountryId ();

				return _get_Plate_plateCountryId;
			}
			set
			{
				_get_Plate_plateCountryId	= value;
			}
		}

		public void loadPlate_plateCountryId (int pageIndex = -1, int pageSize = 100)
		{
			CommandResult	opResult;

			BLL.Logic.GasStation.Plate	logic	= new BLL.Logic.GasStation.Plate (Common.Enum.EDatabase.GasStation);
			if (pageIndex == -1)
				opResult	= logic.allData ("plateCountryId = @plateCountryId", "", false, true, new KeyValuePair ("@plateCountryId", id));
			else
				opResult	= logic.allByPaging ( pageIndex, pageSize, "plateCountryId = @plateCountryId", "", false, true, new KeyValuePair ("@plateCountryId", id));

			if (opResult.status == BaseDAL.Base.EnumCommandStatus.success)
				_get_Plate_plateCountryId	= opResult.model as System.Data.DataTable;
		}
	#endregion
