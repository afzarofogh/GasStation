...
//
	...
	#region Relation - CarOwner (Has-Many relation)
		private System.Data.DataTable _get_CarOwner_ownerId;
		public System.Data.DataTable getCarOwner_ownerId
		{
			get
			{
				if ((_get_CarOwner_ownerId == null) && (AutoLoadForeignKeys))
					loadCarOwner_ownerId ();

				return _get_CarOwner_ownerId;
			}
			set
			{
				_get_CarOwner_ownerId	= value;
			}
		}

		public void loadCarOwner_ownerId (int pageIndex = -1, int pageSize = 100)
		{
			CommandResult	opResult;

			BLL.Logic.GasStation.CarOwner	logic	= new BLL.Logic.GasStation.CarOwner (Common.Enum.EDatabase.GasStation);
			if (pageIndex == -1)
				opResult	= logic.allData ("ownerId = @ownerId", "", false, true, new KeyValuePair ("@ownerId", id));
			else
				opResult	= logic.allByPaging ( pageIndex, pageSize, "ownerId = @ownerId", "", false, true, new KeyValuePair ("@ownerId", id));

			if (opResult.status == BaseDAL.Base.EnumCommandStatus.success)
				_get_CarOwner_ownerId	= opResult.model as System.Data.DataTable;
		}
	#endregion
		...
		[BaseBLL.Base.Field(nullable=false,sqlDBType=System.Data.SqlDbType.VarChar,primary=false,usage=BaseBLL.Base.EnumUsage.read | BaseBLL.Base.EnumUsage.update | BaseBLL.Base.EnumUsage.create,size=10)]
		...
		public Nullable<System.Int32> gen
		{
			get;
			set;
		}
